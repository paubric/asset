...
bool CConsoleHelper::WinUSB_Connect_Default_DPP()
{

	// flag is for notifications, if already connect will return dusbStatNoAction
	// use devInfo.deviceConnected to detect connection
	DPP_USB_STATUS USBStatus;	//0=do nothing, 1=new connect, 2=cannot connect
	WinUSB_isConnected = false;
	WinUSB_NumDevices = 0;
	USBStatus = dusbStatNoAction;
	DppWinUSB.NumDevices = DppWinUSB.CountDP5WinusbDevices();
	if (DppWinUSB.NumDevices > 1) {
		// choose dpp device here or default to first device
	} else if (DppWinUSB.NumDevices == 1) {
		// default to first device
	} else {
	}
	if (DppWinUSB.NumDevices > 0) {
		DppWinUSB.CurrentDevice = 1;	// set to default device
		USBStatus = DppWinUSB.FindUSBDevice(DppWinUSB.CurrentDevice - 1); //connect
		if (DppWinUSB.devInfo.deviceConnected) { // connection detected
			WinUSB_isConnected = true;
			WinUSB_NumDevices = DppWinUSB.NumDevices;
		}
	} else {
		WinUSB_isConnected = false;
		WinUSB_NumDevices = 0;
	}
	return (WinUSB_isConnected);
}

void CConsoleHelper::WinUSB_Close_Connection()
{
	if (DppWinUSB.devInfo.deviceConnected) { // clean-up: close usb connection
...
bool CConsoleHelper::WinUSB_SendCommand(TRANSMIT_PACKET_TYPE XmtCmd)
{
    bool bHaveBuffer;
    bool bSentPkt;
	bool bMessageSent;
	...
	if (DppWinUSB.devInfo.deviceConnected) { 
		...
		if (bHaveBuffer) {
			...
			bSentPkt = DppWinUSB.SendPacketUSB(DP5Proto.BufferOUT, DppWinUSB.devInfo, DP5Proto.PacketIn);
			if (bSentPkt) {
	            bMessageSent = true;
			}
		}
...
// XMTPT_FULL_READ_CONFIG_PACKET (PC5_PRESENT,DppType)
//					Creates and sends a full readback command
bool CConsoleHelper::WinUSB_SendCommand_Config(TRANSMIT_PACKET_TYPE XmtCmd, CONFIG_OPTIONS CfgOptions)
{
    bool bHaveBuffer;
    bool bSentPkt;
	bool bMessageSent;
...
bool CConsoleHelper::WinUSB_ReceiveData()
{
	bool bDataReceived;
