...
#include <string>
#include <vector>
#include "DppWinUSB.h"			// WinUSB Support
...
public:
	...
	/// WinUSB communications class.
	CDppWinUSB DppWinUSB;
	/// WinUSB is connected if true.
	bool WinUSB_isConnected;
	/// WinUSB number of devices found.
	int  WinUSB_NumDevices;
	/// WinUSB connect to the default DPP.
	bool WinUSB_Connect_Default_DPP();
	/// WinUSB close the current connection.
	void WinUSB_Close_Connection();
	/// WinUSB send a command that does not require additional processing.
	bool WinUSB_SendCommand(TRANSMIT_PACKET_TYPE XmtCmd);
	/// WinUSB send a command that requires configuration options processing.
	bool WinUSB_SendCommand_Config(TRANSMIT_PACKET_TYPE XmtCmd, CONFIG_OPTIONS CfgOptions);
	///  WinUSB receive data.
	bool WinUSB_ReceiveData();
