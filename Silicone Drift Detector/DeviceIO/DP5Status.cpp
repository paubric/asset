...
void CDP5Status::Process_Status(DP4_FORMAT_STATUS *m_DP5_Status)
{
    ...
    m_DP5_Status->AccumulationTime = ((float)m_DP5_Status->RAW[12] * 0.001 + (float)(m_DP5_Status->RAW[13] + (float)m_DP5_Status->RAW[14] * 256.0 + (float)m_DP5_Status->RAW[15] * 65536.0) * 0.1)*0.4;
	m_DP5_Status->RealTime = (((double)m_DP5_Status->RAW[20] + ((double)m_DP5_Status->RAW[21] * 256.0) + ((double)m_DP5_Status->RAW[22] * 65536.0) + ((double)m_DP5_Status->RAW[23] * 16777216.0)) * 0.001)*0.4;
    fout<<m_DP5_Status->AccumulationTime<<std::endl<<m_DP5_Status->RealTime<<std::endl;
